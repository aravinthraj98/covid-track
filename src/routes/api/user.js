...
router.get('/', async (req, res) => {
  let loc = await overallcases.find();
  let coordinate = [];
  for (let i in loc) {
    console.log(loc[i].coordinates);
    coordinate.push(loc[i].coordinates[0]);
  }
  ...
  res.render('index.ejs',{coordinate:coordinate});
});
...
router.post('/changepassword', async (req, res) => {
  ...
  if (!isuser) return res.send('not a valid email');
    ...
    if (String(last).length > 2) {
      ...
      if (date < last) {
        console.log('hello');
        return res.send(
          `password has been changed back few days check mail on ${isuser.password_changedon}`
        );
      }
    }
  }
  let ismail = sendmail(
    req.body.mail,
    `your new password is  ${result} change after login`
  );
  if (ismail) {
    try {
      let date = new Date();
      ...
      await User.findOneAndUpdate(
        { email },
        { $set: { password: password, password_changedon: date } }
      );
